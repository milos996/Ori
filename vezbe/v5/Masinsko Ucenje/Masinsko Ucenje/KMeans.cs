...
namespace Masinsko_Ucenje
{
    public class KMeans
    {
        ...
        public void podeliUGrupe(int brojGrupa, double errT)
        {
            ...
            for (int i = 0; i < brojGrupa; i++)
            {
                ...
                Cluster prvi = new Cluster();
                int broj = rnd.Next();
                prvi.centar = elementi[(broj % elementi.Count)];
                grupe.Add(prvi);
            }
    ...
    public class Cluster
    {
        ...
        {   // TODO 6: implementirati funkciju rastojanja
            ...
            double rast = Math.Sqrt(Math.Pow(c.x - centar.x, 2) + Math.Pow(c.y - centar.y, 2));
            return rast;
        }
        ...
        {   // TODO 7: implemenitrati funkciju koja pomera centre klastera
            ...
            double sY = 0;
            double retVal = 0;
            foreach (Point   item in elementi)
            {
                sX += item.x;
                sY += item.y;
            ...
            sX = sX / elementi.Count;
            sY = sY / elementi.Count;

            retVal = Math.Sqrt(Math.Pow(sX - centar.x, 2) + Math.Pow(sY - centar.y, 2));
            centar.x = sX;
            centar.y = sY;
