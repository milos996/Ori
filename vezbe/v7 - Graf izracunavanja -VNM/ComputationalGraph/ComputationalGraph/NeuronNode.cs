...
namespace ComputationalGraph
{
    public class NeuronNode
    {
        ...
        public double forward(List<double> x)
        {
            ...
            {
                double[] inp = { inputs[i], this.multiplyNodes[i].x[1] };
                forSum.Add(this.multiplyNodes[i].forward(inp.ToList()));
            }
        ...
        public List<double> backward(List<double> dz)
        {
            ...
            List<double> sab = this.sumNode.backward(backward_signal);
            for (int i = 0; i < sab.Count; i++)
            {
                dw.Add(this.multiplyNodes[i].backward(sab[i])[1]);
            }
