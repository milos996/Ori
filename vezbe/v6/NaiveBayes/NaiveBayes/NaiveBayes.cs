...
namespace NaiveBayes
{
    public class NaiveBayes
    {
        ...
        public void predict(string text)
        {
            ...
            Pcj_pos = documents_sentiment_count[1] / documentCount;
            Pcj_neg = documents_sentiment_count[0] / documentCount;
            ...
            foreach (KeyValuePair<string, int> item in counts)
            {
                ...
                log_prob_pos += Math.Log(( (word_counts[1].ContainsKey(w) ? (word_counts[1][w] + 1.0)  : 1.0 ) )/ (word_counts[1].Values.Sum() + vocabulary.Count)) *cnt;
                log_prob_neg += Math.Log(((word_counts[0].ContainsKey(w)  ? (word_counts[0][w] + 1.0)  : 1.0) ) / (word_counts[0].Values.Sum() + vocabulary.Count)) * cnt;
            }
            ...
            Console.WriteLine("Pos: " + log_prob_pos.ToString() + "Neg: " + log_prob_neg.ToString() + "Veci je " + (log_prob_neg > log_prob_pos ? "NEGATIVNI" : "POZITIVNI"));
        }
    }
}
